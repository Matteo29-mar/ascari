...
export default function Cars() {
  ...
  // 1) carica lista e bozze all'avvio
  useEffect(() => {
    loadAll();
    setDrafts(loadDrafts());
  }, []);

  // 2) sincronizza le bozze nel DB quando Clerk è pronto
  useEffect(() => {
    if (!isLoaded) return;
    ...
    (async () => {
      try {
        ...
        if (!local.length) return; // niente da sincronizzare

        const token = await getToken();
        if (!token) {
          // utente non loggato: lasciamo le bozze locali
          return;
        }
        ...
        // se va storto non blocchiamo la pagina, semplicemente lasciamo le bozze
      }
    })();
  }, [isLoaded, getToken]);
  ...
  function imagesForCard(car: Car) {
    const fromDb = Array.isArray(car.photos)
      ? car.photos.filter(Boolean) as string[]
      : [];
  ...
  return (
    <div>
                  ...
                  <div className="kv">
                    ...
                    <b>
                      {car.latitude != null && car.longitude != null
                        ? `${car.latitude.toFixed(4)}, ${car.longitude.toFixed(
                            4
                          )}`
                        : '-'}
                    </b>
      ...
      <p className="muted" style={{ marginTop: 18 }}>
        ...
        <code>ascari-gt-1.jpg</code>, <code>ascari-gt-2.jpg</code>, …). Se
        mancano, verrà mostrato <code>placeholder.jpg</code>.
      </p>
    </div>
  );
}
